function gwc -d "Create or update a worktree from upstream/main" -a branch
    test -z "$branch" && echo "Usage: gwc <branch-name>" && return 1
    ...
    if not git rev-parse --is-bare-repository 2>/dev/null | string match -q true
        ...
        if test "$bare_root" != ".git" -a -d "$bare_root"
            set bare_root (dirname "$bare_root")
        else
